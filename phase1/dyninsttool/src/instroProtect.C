...
BPatch_Vector<BPatch_snippet *> createHashFunctionSubSnippet(BPatch_addressSpace *app,
    ...
    BPatch_variableExpr *counter = app->malloc(*(appImage->findType("unsigned long")), "counter");

    BPatch_variableExpr *currentByte =
        app->malloc(*(appImage->findType("unsigned char")), "currentByte");

    BPatch_variableExpr *size = app->malloc(*(appImage->findType("unsigned long")), "size");

    // couter = startAddress
    BPatch_arithExpr *assignCounter =
        new BPatch_arithExpr(BPatch_assign, *counter, BPatch_constExpr(startAddress));

    // result = 0
    BPatch_arithExpr *assignResult =
        new BPatch_arithExpr(BPatch_assign, *result, BPatch_constExpr(hashStartValue));

    // size = size
    BPatch_arithExpr *assignSize =
        new BPatch_arithExpr(BPatch_assign, *size, BPatch_constExpr(blockSize));

    // currentByte = 0
    BPatch_arithExpr *assignCurrentByte =
        new BPatch_arithExpr(BPatch_assign, *currentByte, BPatch_constExpr(0));
    ...
    BPatch_arithExpr *getCurrentByte =
        new BPatch_arithExpr(BPatch_assign, *currentByte, BPatch_arithExpr(BPatch_deref, *counter));
    ...
    // counter < endAddress
    BPatch_boolExpr *counterLEndAddress =
        new BPatch_boolExpr(BPatch_gt, *size, BPatch_constExpr((unsigned long)0));

    // while(counter < endAddress) { whileBody }
    BPatch_whileExpr *whileHash =
        new BPatch_whileExpr(*counterLEndAddress, BPatch_sequence(whileBody));
...
BPatch_Vector<BPatch_snippet *> createHashFunctionAddSnippet(BPatch_addressSpace *app,
                                                             BPatch_variableExpr *result,
                                                             unsigned long startAddress,
                                                             unsigned long blockSize,
                                                             unsigned long hashStartValue) {
...
BPatch_Vector<BPatch_snippet *> createCheckerSnippet(BPatch_addressSpace *app,
                                                     unsigned long correctHash,
                                                     unsigned long startAddress, unsigned long size,
                                                     hashFunction *snippetHashFunction,
                                                     unsigned long hashStartValue = 0) {
    BPatch_image *appImage = app->getImage();

    // Holds all created snippets
    BPatch_Vector<BPatch_snippet *> checkerSnippet;

    BPatch_variableExpr *result = app->malloc(*(appImage->findType("unsigned long")), "result");

    BPatch_variableExpr *correctHashConst =
        app->malloc(*(appImage->findType("unsigned long")), "correctHashConst");

    // correctHashConst = 0
    BPatch_arithExpr *assignCorrectHashConst =
        new BPatch_arithExpr(BPatch_assign, *correctHashConst, BPatch_constExpr(correctHash));

    checkerSnippet.push_back(assignCorrectHashConst);

    // Create a hash function snippet
    BPatch_Vector<BPatch_snippet *> hashFunctionSnippet =
        snippetHashFunction(app, result, startAddress, size, hashStartValue);

    checkerSnippet.insert(std::end(checkerSnippet), std::begin(hashFunctionSnippet),
                          std::end(hashFunctionSnippet));
    ...
    BPatch_ifExpr *checkHash =
        new BPatch_ifExpr(BPatch_boolExpr(BPatch_ne, *result, *correctHashConst), *reportFunction);
...
int blockLengthUntilCall(BPatch_basicBlock *block) {
    ...
    for (Dyninst::InstructionAPI::Instruction::Ptr inst : insns) {
        if (instructionUsesAddress(inst)) {
            break;
        }
        length += inst->size();
    }
...
unsigned long computeHash(BPatch_basicBlock *block,
                          unsigned long (*hashFunction)(std::vector<unsigned long>, unsigned long),
...
Graph createCheckerNetwork(BPatch_addressSpace *app, int connectivity,
                           std::vector<std::string> functions) {
    // Creates a checker network for all functions of an app listen in the given vector
    // Vertices in the network are basic blocks of the functions and edges are relations checker ->
    // basic block
    Graph g;
    BPatch_image *appImage = app->getImage();
    ...
    for (auto name : functions) {
        ...
        if (appImage->findFunction(name.c_str(), funcs) != NULL) {
            for (auto singleFunc : funcs) {
                // Splits a function in its basic blocks and adds them as vertices to the defined
                // graph
    ...
    if (connectivity > (int)vertices.size() - 1) {
        cout << "WARNING" << endl;
        cout << "The specified connectivity " << connectivity
             << " is larger than the number of basic blocks." << endl;
...
int main(int argc, char *argv[]) {
    ...
    for (vp = vertices(g); vp.first != vp.second; ++vp.first) {
        ...
        cout << "Block " << v << " with startAddress " << hex << "0x"
             << basicBlock->getStartAddress() << " checks:" << endl;
        ...
        for (adj_vp = adjacent_vertices(v, g); adj_vp.first != adj_vp.second; ++adj_vp.first) {
            ...
            cout << "\tBlock " << v << " with startAddress "
                 << "0x" << blockToCheck->getStartAddress() << endl;
            ...
            BPatch_Vector<BPatch_snippet *> currentCheckerSnippet =
                createCheckerSnippet(app, correctHash, blockToCheck->getStartAddress(),
                                     blockLengthUntilCall(blockToCheck), hashFunctionSnippet);

            checkerSnippet.insert(std::end(checkerSnippet), std::begin(currentCheckerSnippet),
                                  std::end(currentCheckerSnippet));
        }
        ...
        if (vp.first != vp.second - 1) {
            ...
            BPatch_Vector<BPatch_snippet *> currentCheckerCheckSnippet = createCheckerSnippet(
                app, PLACEHOLDER, PLACEHOLDER, PLACEHOLDER, hashFunctionSnippet);
            checkerSnippet.insert(std::end(checkerSnippet), std::begin(currentCheckerCheckSnippet),
                                  std::end(currentCheckerCheckSnippet));
        }
