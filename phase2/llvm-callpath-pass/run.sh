...
opt-3.9 -load "${build}libCallPathProtectorPass.so" \
				-callpath -ff $filename -o "${build}${cfile}-inst.bc" "${build}${cfile}.bc"
exitIfFail $?

llc-3.9 -filetype=obj "${build}${cfile}-inst.bc"
exitIfFail $?

g++ -rdynamic "${build}${cfile}-inst.o" "${build}libcheck.o" "${build}crypto.o" \
				-o "${build}${cfile}-rewritten" -L/usr/lib/x86_64-linux-gnu/ -lssl -lcrypto
exitIfFail $?
