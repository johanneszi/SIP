...
namespace {
    ...
    struct CallPathProtectorPass : public ModulePass {
        ...
        Type *ptrTy, *voidTy, *boolTy;  // These variables are to store the type
    ...
    bool CallPathProtectorPass::doInitialization(Module &M) {
        ...
        checkFunction = M.getOrInsertFunction(
            CHECKFUNC, FunctionType::get(boolTy, ArrayRef<Type *>(argsTypes), false));

        reportFunction =
            M.getOrInsertFunction(REPORTFUNC, FunctionType::get(voidTy, boolTy, false));
    ...
    bool CallPathProtectorPass::runOnModule(Module &M) {
        ...
        for (auto funcName : functionsToProtect) {
            ...
            if (func == nullptr || func->size() <= 0) {
                errs() << WARNING << " Function " << funcName
    ...
    VecInVec CallPathProtectorPass::getCall(CallGraph *CG, StringRef func,
