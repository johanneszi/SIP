...
typedef struct ms_collide_gold_t {
	int ms_retval;
	snake_t* ms_snake;
	screen_t* ms_screen;
} ms_collide_gold_t;
...
static sgx_status_t SGX_CDECL sgx_collide_gold(void* pms)
{
	ms_collide_gold_t* ms = SGX_CAST(ms_collide_gold_t*, pms);
	...
	CHECK_REF_POINTER(pms, sizeof(ms_collide_gold_t));
	...
	ms->ms_retval = collide_gold(_in_snake, _in_screen);
err:
	if (_in_snake) free(_in_snake);
	if (_in_screen) free(_in_screen);
...
SGX_EXTERNC const struct {
	...
	4,
	{
		{(void*)(uintptr_t)sgx_collide_gold, 0},
